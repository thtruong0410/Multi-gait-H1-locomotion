...
class RewardThresholdCurriculum(Curriculum):
    ...
    def update(self, bin_inds, task_rewards, success_thresholds, local_range=0.5):
        if torch.is_tensor(bin_inds):
            bin_inds = bin_inds.cpu().numpy()
        bin_inds = bin_inds.astype(int)

        is_success = 1.
        for task_reward, success_threshold in zip(task_rewards, success_thresholds):
            if torch.is_tensor(task_reward):
                task_reward = task_reward.cpu()
            is_success = is_success * (task_reward > success_threshold).float()

        if len(success_thresholds) == 0:
            is_success = np.array([False] * len(bin_inds))
        else:
            is_success = is_success.bool().cpu().numpy()
...
def terrain_levels_vel(
    ...
    move_up = distance > terrain.cfg.terrain_generator.size[0] * 0.5
