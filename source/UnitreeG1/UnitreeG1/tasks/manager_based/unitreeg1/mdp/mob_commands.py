...
class MoBCommandPlay(UniformVelocityCommand):
    cfg: MoBCommandCfg
    ...
    def __init__(self, cfg: MoBCommandCfg, env: ManagerBasedEnv):
        ...
        self.all_commands = torch.zeros(self.num_envs, 15, device= self.device)
        CurriculumClass = RewardThresholdCurriculum
        self.curricula = []
        for category in self.category_names:
            self.curricula += [CurriculumClass(seed=cfg.seed,
                                                x_vel=(cfg.limit_vel_x[0],
                                                    cfg.limit_vel_x[1],
                                                    cfg.num_lin_vel_x),

                                                y_vel=(cfg.limit_vel_y[0],
                                                    cfg.limit_vel_y[1],
                                                    cfg.num_lin_vel_y),

                                                yaw_vel=(cfg.limit_vel_yaw[0],
                                                        cfg.limit_vel_yaw[1],
                                                        cfg.num_ang_vel_z),

                                                body_height=(cfg.limit_body_height[0],
                                                            cfg.limit_body_height[1],
                                                            cfg.num_body_height),

                                                gait_frequency=(cfg.limit_gait_frequency[0],
                                                                cfg.limit_gait_frequency[1],
                                                                cfg.num_gait_frequency),

                                                gait_phase=(cfg.limit_gait_phase[0],
                                                            cfg.limit_gait_phase[1],
                                                            cfg.num_gait_phase),

                                                gait_offset=(cfg.limit_gait_offset[0],
                                                            cfg.limit_gait_offset[1],
                                                            cfg.num_gait_offset),

                                                gait_bounds=(cfg.limit_gait_bound[0],
                                                            cfg.limit_gait_bound[1],
                                                            cfg.num_gait_bounds),

                                                gait_duration=(cfg.limit_gait_duration[0],
                                                            cfg.limit_gait_duration[1],
                                                            cfg.num_gait_duration),

                                                footswing_height=(cfg.limit_footswing_height[0],
                                                                cfg.limit_footswing_height[1],
                                                                cfg.num_footswing_height),

                                                body_pitch=(cfg.limit_body_pitch[0],
                                                            cfg.limit_body_pitch[1],
                                                            cfg.num_body_pitch),

                                                body_roll=(cfg.limit_body_roll[0],
                                                        cfg.limit_body_roll[1],
                                                        cfg.num_body_roll),

                                                stance_width=(cfg.limit_stance_width[0],
                                                            cfg.limit_stance_width[1],
                                                            cfg.num_stance_width),

                                                stance_length=(cfg.limit_stance_length[0],
                                                            cfg.limit_stance_length[1],
                                                            cfg.num_stance_length),

                                                aux_reward_coef=(cfg.limit_aux_reward_coef[0],
                                                                cfg.limit_aux_reward_coef[1],
                                                                cfg.num_aux_reward_coef),
        ...
        self.env_command_categories = torch.zeros(self.num_envs, device= self.device)
        low = np.array(
            [cfg.ranges.lin_vel_x[0], cfg.ranges.lin_vel_y[0],
             cfg.ranges.ang_vel_z[0], cfg.ranges.body_height_range[0],
             cfg.ranges.gait_frequency_range[0],
             cfg.ranges.gait_phase_range[0], cfg.ranges.gait_offset_range[0],
             cfg.ranges.gait_bounds_range[0], cfg.ranges.gait_duration_range[0],
             cfg.ranges.footswing_height_range[0], cfg.ranges.body_pitch_range[0],
             cfg.ranges.body_roll_range[0],cfg.ranges.stance_width_range[0],
             cfg.ranges.stance_length_range[0], cfg.ranges.aux_reward_coef_range[0], ])
        high = np.array(
            [cfg.ranges.lin_vel_x[1], cfg.ranges.lin_vel_y[1],
             cfg.ranges.ang_vel_z[1], cfg.ranges.body_height_range[1],
             cfg.ranges.gait_frequency_range[1],
             cfg.ranges.gait_phase_range[1], cfg.ranges.gait_offset_range[1],
             cfg.ranges.gait_bounds_range[1], cfg.ranges.gait_duration_range[1],
             cfg.ranges.footswing_height_range[1], cfg.ranges.body_pitch_range[1],
             cfg.ranges.body_roll_range[1],cfg.ranges.stance_width_range[1],
             cfg.ranges.stance_length_range[1], cfg.ranges.aux_reward_coef_range[1], ])
        for curriculum in self.curricula:
            curriculum.set_to(low=low, high=high)
        self.gait_indices = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
        self.clock_inputs = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device, requires_grad=False)
        self.foot_indices = torch.tensor([0, 1, 2, 3], dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.all_commands[env_ids, 0] = r.uniform_(*self.cfg.ranges.lin_vel_x)
        self.all_commands[env_ids, 1] = r.uniform_(*self.cfg.ranges.lin_vel_y)
        self.all_commands[env_ids, 2] = r.uniform_(*self.cfg.ranges.ang_vel_z)
        self.all_commands[env_ids, 3] = r.uniform_(*self.cfg.ranges.body_height_range)        
        self.all_commands[env_ids, 4] = r.uniform_(*self.cfg.ranges.gait_frequency_range)        
        self.all_commands[env_ids, 5] = r.uniform_(*self.cfg.ranges.gait_phase_range)        
        self.all_commands[env_ids, 6] = r.uniform_(*self.cfg.ranges.gait_offset_range)        
        self.all_commands[env_ids, 7] = r.uniform_(*self.cfg.ranges.gait_bounds_range)        
        self.all_commands[env_ids, 8] = r.uniform_(*self.cfg.ranges.gait_duration_range)        
        self.all_commands[env_ids, 9] = r.uniform_(*self.cfg.ranges.footswing_height_range)        
        self.all_commands[env_ids, 10] = r.uniform_(*self.cfg.ranges.body_pitch_range)        
        self.all_commands[env_ids, 11] = r.uniform_(*self.cfg.ranges.body_roll_range)        
        self.all_commands[env_ids, 12] = r.uniform_(*self.cfg.ranges.stance_width_range)        
        self.all_commands[env_ids, 13] = r.uniform_(*self.cfg.ranges.stance_length_range)  
    ...
    def _step_contact_targets(self):
        frequencies = self.all_commands[:, 4]
        phases = self.all_commands[:, 5]
        offsets = self.all_commands[:, 6]
        bounds = self.all_commands[:, 7]
        durations = self.all_commands[:, 8]
        # print(self.env.step_dt)
        self.gait_indices = torch.remainder(self.gait_indices + frequencies * self.env.step_dt, 1.0)
        ...
        # print("in step" ,self.desired_contact_states)
        # print(smoothing_multiplier_FL[0], smoothing_multiplier_FR[0], smoothing_multiplier_RL[0], smoothing_multiplier_RR[0])
        self.desired_footswing_height = self.all_commands[:, 9]
    ...
    def _update_metrics(self):
        ...
        # logs data
        self.metrics["error_vel_xy"] += (
            torch.norm(self.all_commands[:, :2] - self.robot.data.root_lin_vel_b[:, :2], dim=-1) / max_command_step
        )
        self.metrics["error_vel_yaw"] += (
            torch.abs(self.all_commands[:, 2] - self.robot.data.root_ang_vel_b[:, 2]) / max_command_step
        )
