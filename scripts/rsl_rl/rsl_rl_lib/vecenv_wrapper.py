...
class RslRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ManagerBasedRLEnv | DirectRLEnv, clip_actions: float | None = None):
        ...
        self.num_one_step_obs = self.unwrapped.num_one_step_obs if hasattr(self.unwrapped, "num_one_step_obs") else 0
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        # print(terminated)
        terminated_ids = torch.nonzero(terminated, as_tuple=False).squeeze(-1)
        # print(terminated_ids)
        # move extra observations to the extras dict
        obs = obs_dict["policy"]
        # print("obs: ", obs.shape)
        # print("obs_dict policy: ", obs_dict["policy"].shape)
        # print("obs_dict critic: ", obs_dict["critic"].shape)
        ...
        termination_privileged_obs = privileged_obs
        termination_privileged_obs = termination_privileged_obs[terminated_ids]

        # return the step information
        return obs, privileged_obs, rew, dones, extras, terminated_ids, termination_privileged_obs 
