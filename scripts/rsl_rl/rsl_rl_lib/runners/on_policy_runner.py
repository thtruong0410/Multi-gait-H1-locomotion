...
import rsl_rl
from rsl_rl_lib.algorithms import PPO
from rsl_rl_lib.env import VecEnv
from rsl_rl_lib.modules import ActorCritic
from rsl_rl_lib.utils import store_code_state
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        print("policy_ cfg ", self.policy_cfg)
        # initialize algorithm
        policy = ActorCritic(num_obs, num_privileged_obs, self.env.num_actions, **self.policy_cfg).to(self.device)
        self.alg_cfg.pop("class_name", None)
        self.alg = PPO(policy, device=self.device, **self.alg_cfg)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from rsl_rl_him.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from rsl_rl_him.utils.wandb_utils import WandbSummaryWriter
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    obs, privileged_obs, rewards, dones, infos, terminated_ids, termination_privileged_obs = self.env.step(actions)
